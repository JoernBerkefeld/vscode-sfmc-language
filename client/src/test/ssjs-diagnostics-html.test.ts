/**
 * ssjs-diagnostics-html.test.ts — VS Code integration tests for SSJS LSP diagnostics
 * inside an HTML <script runat="server"> block.
 *
 * Mirrors ssjs-diagnostics.test.ts but uses test-ssjs-requires-load-html.html.
 */
...
suite('SSJS-in-HTML Diagnostics — requiresCoreLoad globals', () => {
    const documentUri = getDocumentUri('test-ssjs-requires-load-html.html');
    ...
    test('Stringify() before Platform.Load is still flagged (order-aware check)', () => {
        // Section B has a bare Stringify() BEFORE Platform.Load.
